...
# Setting up status bar/report
# print("\nCurrent operation  \t\t\t\tTotal Completion",flush=True)
# print("\nConnecting to node...\t\t\t\t", end="",flush=True)

# Dude abides (just so I can compare outputs to my code)
...
def Ask_Node(command):
    full_command = ["bitcoin-cli"] + get_cli_options() + command
...
if block_mode:
    ...
    print("    Finding all blocks on "+datetime_entered.strftime("%b %d, %Y"),flush=True)
